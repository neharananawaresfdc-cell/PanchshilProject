...
import buttonPANandAadhar from '@salesforce/apex/EYI_PAN_AadharVerificationLwcController.buttonPANandAadhar';
...
export default class Ey_PAN_AadharVerificationLwc extends NavigationMixin(LightningElement){
    @track confettiArray = [];
   ...
   setPageReference(currentPageReference) {
       ...
       if (currentPageReference) {
        ...
        this.recId = currentPageReference.state.recId;
           ...
           if (this.currentUrl.includes('EYI_REC__c')) {
           ...
           if(this.recAccId){
              buttonPANandAadhar({recAccId:this.recAccId}).
              then(result => {
                  console.log('result-->'+JSON.stringify(result));
                  console.log('result.is_PAN_E_KYC_Completed__c'+result.is_PAN_E_KYC_Completed__c);
                  if(result){
                    this.recAccountDetails = true;
                    if(result.is_PAN_E_KYC_Completed__c) this.panKYCCompleted = result.is_PAN_E_KYC_Completed__c;
                    if(result.is_Aadhar_E_KYC_Completed__c) this.aadharKYCCompleted = result.is_Aadhar_E_KYC_Completed__c;
                    if(result.EYI_First_Name__c) this.fname = result.EYI_First_Name__c;
                    if(result.EYI_Last_Name__c) this.lname = result.EYI_Last_Name__c;
                    
                    if(result.Name) this.companyCode = result.EYI_REC_Code__c; 
                    if(result.EYI_Mobile__c) this.mobileNo = result.EYI_Mobile__c; 
                    ...
                    if(result.EYI_PAN_No__c) this.panNumber = result.EYI_PAN_No__c;
                    if(result.EYI_Aadhar__c) this.aadharNumber = result.EYI_Aadhar__c;
   ...
   handlePreview(event){
    ...
    this[NavigationMixin.Navigate]({
        ...
        state: {
            selectedRecordId: contentDocumentID // Use ContentDocumentId, NOT ContentVersionId
        }
    })
}
