...
import getRecords from '@salesforce/apex/EYI_DDT_RelatedList.getRecords';
import onSearch from '@salesforce/apex/EYI_DDT_RelatedList.onSearch';
import buildFieldJSON from '@salesforce/apex/EYI_DDT_RelatedList.buildFieldJSON';
import getRecordsWS from '@salesforce/apex/EYI_DDT_RelatedList_WS.getRecords';
import onSearchWS from '@salesforce/apex/EYI_DDT_RelatedList_WS.onSearch';
import buildFieldJSONWS from '@salesforce/apex/EYI_DDT_RelatedList_WS.buildFieldJSON';
...
import {
    ...
    formatData,
    _formatData
} from './eyiDdtMainLwcHelper';
import recordUpdatedSuccessMessage from '@salesforce/label/c.eyiRecordUpdatedSuccessMessage';
import recordDeletedSuccessMessage from '@salesforce/label/c.eyiRecordDeletedSuccessMessage';
...
export default class LwcDatatable extends NavigationMixin(LightningElement) {
                ...
                .then((data) => {
                    if (data) {
                        ...
                        colAc.push({
                         ...
                         console.log('column ch1',JSON.stringify(this.colsJson),JSON.stringify(colAc));
                        //rem
                        ...
                        this.columns = colAc.map(col => {
                        if (col.fieldName) {
                            return { ...col, initialWidth: 150 };
                        }
                        return col;
                        });

                        console.log('column ch2',this.columns,JSON.stringify(this.columns));
                        this.data = records;
    ...
    formatError(error) {
        if (error) {
            console.error('SOQL', this.soql, 'error', error);
    ...
    appendWhere() {
        ...
        if (this.whereClause) {
            ...
            if (this.whereClause.includes('recordId')) {
                console.log('where clause in rId', this.whereClause);
                this.whereClause = this.whereClause.replaceAll('recordId', this.recordId);
                console.log('where clause out rId', this.whereClause, this.recordId);
            }
            if (this.whereClause.includes('connectedUserId')) {
                this.whereClause = this.whereClause.replaceAll('connectedUserId', this.connectedUserId);
            }
